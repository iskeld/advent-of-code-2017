defmodule Advent do
  ...
  def solve(input) do
    inputs(input)
    |> Enum.map(&used_count/1)
    |> Enum.sum()
  end

  def inputs(src), do: Enum.map(0..127, &("#{src}-#{&1}"))

  def used_count(input), do: hash(input) |> to_binary() |> ones_count()

  def ones_count(binary), do: Enum.count(String.to_charlist(binary), fn x -> x == ?1 end)
   
  def to_binary(hash), do: Integer.parse(hash, 16) |> elem(0) |> Integer.to_string(2)
