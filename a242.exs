defmodule Advent do
  ...
  def step(port, components, edges, len, strength) do
    ...
    result = Enum.reduce(matches, {:empty, edges}, fn ({side_port, rest}, {_, acc}) ->
      {:ok, step(side_port, rest, acc, len + 1, port + current_strength)}
    end)

    case result do
      {:empty, _} -> [{len, current_strength} | edges]
